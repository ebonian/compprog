def is_inputs_valid(n_a, n_b, total_rows, seats_per_row, policy):
    ...
    # Your code here
    if not (n_a in range(1, 21) and n_b in range(1, 21)):
        print(error_student_number)
        return False

    elif not (total_rows >= 4):
        print(error_total_rows)
        return False
    
    elif not (seats_per_row == 4 or seats_per_row == 6 or seats_per_row == 8):
        print(error_seats_per_row)
        return False

    elif not (total_rows * seats_per_row > n_a + n_b):
        print(error_not_enough_seat)
        return False

    elif not (policy == 0 or policy == 1):
...
def generate_seat_ids(subject_code, n):
    ...
    for i in range(1, n + 1):
        sequence = "0" + str(i) if len(str(i)) == 1 else str(i)

        seat_id = subject_code + sequence
...
def generate_row_seating_sequence(group1, group2, total_rows, seats_p_row):
    ...
    # assisgn longer_group and shorter_group by group1 and group2 length
    longer_group = group1
    shorter_group = group2


    # fill empty seat with "-"
    for i in range(int((total_rows * seats_p_row) / 2)):
        if (i >= len(longer_group)):
            longer_group.append(" - ")
        if (i >= len(shorter_group)):
            shorter_group.append(" - ")


    # merge group1 and group2 alternatively to seating_sequence
    for longer_group_seat, shorter_group_seat in zip(longer_group, shorter_group):
        seating_sequence.append(longer_group_seat)
        seating_sequence.append(shorter_group_seat)
...
def generate_column_seating_sequence(group1, group2, total_rows, seats_p_row):
    ...
    # reassign group1 and group2 to empty list
    group1 = []
    group2 = []
    ...
    # append each seat step by total rows to group 1 and group 2
    for i in range(total_rows):
        for j in longer_group[i::total_rows]:
            group1.append(j)
        for k in shorter_group[i::total_rows]:
            group2.append(k)


    # merge group1 and group2 alternatively to seating_sequence
    for group1_seat, group2_seat in zip(group1, group2):
        seating_sequence.append(group1_seat)
        seating_sequence.append(group2_seat)
...
def display_seating_map(seating_sequence, total_rows, seats_per_row):
    ...
    # Your code here
    table_length = ((seats_per_row) * 3) + seats_per_row + 1

    row_divider = "-" * table_length

    space_between_header = " " * int((table_length - 11) / 2)

    print(row_divider)
    print("|" + space_between_header + "Exam Room" + space_between_header + "|")
    print(row_divider)

    for i in range(1, len(seating_sequence) + 1):
        seat = seating_sequence[i - 1]
