...
def top_k_min_temp_stations(stations, K):
    l=[]
    for x in stations:
        l.append([stations[x]['temp'],x])
    l.sort()
    return [x[1] for x in l[:K]]
...
def peak_stations(stations):
    lati,longi=[],[]
    for x in stations:
        lati.append([stations[x]['lat'],x])
        longi.append([stations[x]['long'],x])
    return [min(lati)[1],max(lati)[1],min(longi)[1],max(longi)[1]]

def k_nearby_stations(stations, main_station, K):
    lat0,long0,l=stations[main_station]['lat'],stations[main_station]['long'],[]
    
    for x in stations:
        lat,long=stations[x]['lat'],stations[x]['long']
        l.append([distance(lat,long,lat0,long0),x])
    return [x[1] for x in sorted(l)[1:K+1]]
def average_temp(stations, names):
    import statistics as s
    l=[]
    for x in names:
        l.append(stations[x]['temp'])
    return s.mean(l)
